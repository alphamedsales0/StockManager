  ...
  <v-container>
        ...
        <template #top>
          <div class="bg-grey-lighten-3 py-3 px-4 d-flex align-center flex-wrap" style="gap: 12px;">
            <div class="d-flex align-center">
              ...
              <span class="text-subtitle-1 font-weight-medium">Vorhandene Typen</span>
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="saveType" :loading="saving" class="text-none">Speichern</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card>
        ...
        <v-card-text class="pt-4">
          Möchten Sie den Artikeltyp <strong class="text-error">{{ selectedItem?.name }}</strong> wirklich löschen?
          Diese Aktion kann nicht rückgängig gemacht werden.
        </v-card-text>
        <v-card-actions class="pa-4">
          <v-btn variant="text" @click="deleteDialog=false">Abbrechen</v-btn>
          <v-btn color="error" @click="deleteType" :loading="deleting" class="text-none">Löschen</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const saveType = async () => {
  ...
  saving.value = true
  try {
    const url = editing.value ? '/api/update_article_type.php' : '/api/add_article_type.php'
...
const deleteType = async () => {
  ...
  try {
    const response = await axios({
      method: 'post',
      url: '/api/delete_article_type.php',
